...
parser = argparse.ArgumentParser(description="compare simulated surface displacement with GPS")
...
for i in range(0,np.size(df,0)-1):
    if ((df["lon"].to_numpy()[i] > extentmap[0]) and (df["lon"].to_numpy()[i] < extentmap[1]) and (df["lat"].to_numpy()[i] > extentmap[2]) and (df["lat"].to_numpy()[i] < extentmap[3])):      
        if df["station"][i] == 'AKLE':
            ax[0].text(df["lon"].to_numpy()[i]-0.28, df["lat"].to_numpy()[i]+0.07,df["station"][i],font=fpath,color=[0.2,0.2,0.2])
        else:
